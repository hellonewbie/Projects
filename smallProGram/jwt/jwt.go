...
type JwtParam struct {
	DefaultIss      string //默认值
	DefaultAudience string //默认音频
	DefaultJti      string
	SecretKey       string
	TokenKey        string
...
func CreateToken(userIdString string) (token string, err error) {
	...
	if err != nil {
		utils.Mylog.Error("content", "token create error", "error", err.Error())
		return "", err
	}

	err = jwtParam.RedisCache.Set(userIdString, tokenString, jwtParam.TokenLife).Err()
	if err != nil {
		utils.Mylog.Error("content", "token create error", "error", err.Error())
		return "", err
	}
...
func ParseToken(myToken string) (userId string, err error) {
	...
	if err != nil {
		utils.Mylog.Error("content", "parse token has error", "error", err.Error())
		return "", err
	}

	if !token.Valid {
		utils.Mylog.Error("content", "token is invalid", "error", err.Error())
		return "", err
	}
	...
	if !ok {
		utils.Mylog.Error("content", "claims duan yan is error", "error", err.Error())
		return "", errors.New("claims duan yan is error")
	}

	res, err := jwtParam.RedisCache.Get(sub).Result()

	if err != nil {
		utils.Mylog.Error("content", "get token from redis error", "error", err.Error())
		return "", err
	}
	...
	err = jwtParam.RedisCache.Set(sub, myToken, jwtParam.TokenLife).Err()
	if err != nil {
		utils.Mylog.Error("content", "token create error", "error", err.Error())
		return "", err
	}
...
func UnsetToken(myToken string) (bool, error) {
	...
	if err != nil {
		utils.Mylog.Error("content", "unset token has error", "error", err.Error())
		return false, err
	}
	return true, nil
}
